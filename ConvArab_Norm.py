...
print(norm_counts.head())
aver_log_normcounts = np.average(norm_counts.values, axis=0)
sorted_avg = np.sort(aver_log_normcounts, axis=None)
threshold = np.average(sorted_avg, axis=None)
...
for promoters, ids, genome in zip(proms_all_genomes, IDS_all_genomes, key_all_genomes):
    encoded_prom = []
    label = [0 if df[genome][gene] < threshold else 1 for gene in ids]
...
model.add(Activation('relu'))
model.add(MaxPool2D(pool_size=(1, 8), padding='same', strides=(1, 8)))
